...
public class DataInitializer {
	...
	private void outputDataForCNNIntoFile(Map<String, Map<String, Integer>> labelsClassData,
		...
		for (Map.Entry<String, List<Map<String, List<String>>>> featureData : featuresData.entrySet()) {
			...
			String fileNameOfFeature = featureData.getKey();
			fileNameOfFeature = fileNameOfFeature.substring(0, fileNameOfFeature.lastIndexOf(".list"));
			...
			for (Map.Entry<String, Map<String, Integer>> labelData : labelsClassData.entrySet()) {
				String fileName = labelData.getKey();
				if (fileName.contains(fileNameOfFeature)) {
					...
					for (Map<String, List<String>> feature : features) {
						...
						if (numberOfData % 1000 == 0) {
							FileHelper.outputToFile(TRAINING_FILE_PATH + fileName, sb);
							sb = new StringBuilder();
						}
					}
					if (sb.length() > 0) {
						FileHelper.outputToFile(TESTING_FILE_PATH + fileName, sb);
					}
				}
			}
		}
